...
local Settings = KODlib.Settings
local Flags = KODlib.Flags

local SetProps, SetChildren, InsertTheme, Create do
    InsertTheme = function(Instance, Type)
        table.insert(KODlib.Instances, {
            Instance = Instance,
            Type = Type
        })
        return Instance
    ...
    local function Save(file)
        if readfile and isfile and isfile(file) then
            ...
            if type(decode) == "table" then
                if rawget(decode, "UISize") then KODlib.Save["UISize"] = decode["UISize"] end
                if rawget(decode, "TabSize") then KODlib.Save["TabSize"] = decode["TabSize"] end
                if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then KODlib.Save["Theme"] = decode["Theme"] end
            end
        end
    end
    
    pcall(Save, "KOD library v8.1.json")
end
...
local Connections, Connection = {}, KODlib.Connection do
...
local function VerifyTheme(Theme)
    for name,_ in pairs(KODlib.Themes) do
        if name == Theme then
            return true
        end
    end
...
local Theme = KODlib.Themes[KODlib.Save.Theme]

local function AddEle(Name, Func)
    KODlib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
    local Element = KODlib.Elements[Ele](Instance, props, ...)
    return Element
end
...
function KODlib:SetTheme(NewTheme)
    if not VerifyTheme(NewTheme) then return end
    
    KODlib.Save.Theme = NewTheme
    SaveJson("KOD library v8.1.json", KODlib.Save)
    
    local OldTheme = Theme
    Theme = KODlib.Themes[NewTheme]
    ...
    for _, Val in pairs(KODlib.Instances) do
    ...
    for _, TabData in pairs(KODlib.Tabs) do
...
function KODlib:SetScale(NewScale)
...
function KODlib:MakeWindow(Configs)
    local WTitle = Configs[1] or Configs.Name or Configs.Title or "KOD Hub"
    ...
    local UISizeX, UISizeY = unpack(KODlib.Save.UISize)
    ...
    local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
        Size = UDim2.new(0, KODlib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
    ...
    ConnectSave(ControlSize1, function()
        if not Minimized then
            KODlib.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
            SaveJson("KOD library v8.1.json", KODlib.Save)
        end
    end)
    
    ConnectSave(ControlSize2, function()
        KODlib.Save.TabSize = MainScroll.Size.X.Offset
        SaveJson("KOD library v8.1.json", KODlib.Save)
    end)
    ...
    local function UpdateSearch(Query)
        ...
        for _, ElementData in pairs(KODlib.AllElements) do
    ...
    function Window:SelectTab(TabSelect)
        if type(TabSelect) == "number" then
            KODlib.Tabs[TabSelect].func:Enable()
        else
            for _,Tab in pairs(KODlib.Tabs) do
    ...
    function Window:MakeTab(paste, Configs)
        ...
        local function Tabs()
            ...
            table.foreach(KODlib.Tabs, function(_,Tab)
        ...
        FirstTab = true
        local Tab = {}
        table.insert(KODlib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
        Tab.Cont = Container
        ...
        function Tab:AddSection(Configs)
            ...
            table.insert(KODlib.AllElements, {
                Name = SectionName,
            ...
            local Section = {}
            table.insert(KODlib.Options, {type = "Section", Name = SectionName, func = Section})
    ...
    task.spawn(function()
        ...
        ToggleGui.Name = "KOD_Hub_Toggle_Circle"
...
local function CreateNotification(Icon, Title, Message, Duration)
    ...
    TitleLabel.Text = Title or "KOD Hub"
...
function KODlib:SetUIBorderColor(color)
    if typeof(color) == "Color3" then
        self.Themes[self.Save.Theme]["UI Border Color"] = color
        if self.Save.Theme == KODlib.Save.Theme then
...
function KODlib:SetDiscordBorderColor(color)
    if typeof(color) == "Color3" then
...
function KODlib:SetProfileBorderColor(color)
    if typeof(color) == "Color3" then
...
function KODlib:SetHubColor(color)
    if typeof(color) == "Color3" then
...
function KODlib:SetStrokeColor(color)
    if typeof(color) == "Color3" then
...
function KODlib:SetTextColor(color)
    if typeof(color) == "Color3" then
...
function KODlib:SetToggleOnColor(color)
    if typeof(color) == "Color3" then
...
function KODlib:SetToggleOffColor(color)
    if typeof(color) == "Color3" then
...
function KODlib:SetToggleKnobOnColor(color)
    if typeof(color) == "Color3" then
...
function KODlib:SetToggleKnobOffColor(color)
    if typeof(color) == "Color3" then
...
function KODlib:SetToggleBorderColor(color)
    if typeof(color) == "Color3" then
...
function KODlib:SetBorderThickness(thickness)
...
function KODlib:GetBorderColors()
    return {
...
function KODlib:GetToggleColors()
    return {
...
function KODlib:ResetBorderColors()
...
function KODlib:ResetToggleColors()
    local defaultTheme = self.Themes.QuangHuy
...
task.spawn(function()
    ...
    KODlib:Notify({
        Title = "KOD Library",
        Message = "UI KOD Library Download Completed",
        Duration = 10
    })
end)
